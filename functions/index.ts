...
const {
	...
	refresh_unknown_and_unscanned_codes,
...
exports.requestManualCodeRefresh = v1_oncall(
	["memory_512MB", "long_timeout"],
	refresh_unknown_and_unscanned_codes,
);
...
exports.refreshScannedCodesStatuses = v1_oncall(
	["memory_512MB", "long_timeout"],
	refreshScannedCodesStatuses,
);
...
exports.recalculate_available_codes_on_event_registration = functions
	.runWith({ timeoutSeconds: 540, memory: "4GB" })
	.firestore.document(`events/{eventId}`)
	.onCreate(({ params }) => recalculate_available_codes(params.eventId));
...
exports.clean_up_expired_items = functions
	.runWith({ timeoutSeconds: 540, memory: "4GB" })
	.pubsub.schedule("every 24 hours")
	.onRun(clean_up_expired_items);
exports.delete_data_of_deleted_event = functions.firestore
	.document(`events/{eventId}`)
	.onDelete(delete_data_of_deleted_event);

// Update items where parents were updated
exports.updatePublicEventData = functions.firestore
	.document(`events/{eventId}`)
	.onWrite(updatePublicEventData);
exports.updateEventAvailableCodes = functions.firestore
	.document(`codes/{codeId}`)
	.onUpdate(updateEventAvailableCodes);
...
const {
	export_emails_of_static_drop,
	create_static_drop,
	update_public_static_drop_data,
	delete_emails_of_static_drop,
} = require("./modules/static_qr_drop");
...
exports.update_public_static_drop_data = functions.firestore
	.document(`static_drop_private/{drop_id}`)
	.onWrite(update_public_static_drop_data);
