...
const log_scan = async (req, data) => {
	...
	await Promise.all([
		...
		db
			.collection("events")
			.doc(data.event_id)
			.set({ scans: increment(1) }, { merge: true }),
	]);
};
...
app.get("/claim/:event_id/:public_auth_token", async (req, res) => {
	...
	try {
		...
		if (dev)
			log(
				`CI: ${CI}, FORCE_INVALID_APPCHECK: ${FORCE_INVALID_APPCHECK}, redirect_baseurl: ${redirect_baseurl}`,
			);
		...
		const previous_auth_within_grace_period =
			public_auth?.created > Date.now() - old_auth_grace_period_in_ms;
		...
		if (completely_invalid || outside_grace_period) {
			...
			if (dev) {
				// Log timing data
				log(
					`Grace period for ${CI ? "CI" : "non-CI"}, event ${
						is_test_event ? "is" : "is not"
					} mock/test event, old_auth_grace_period_in_ms: ${old_auth_grace_period_in_ms}`,
				);
				log(
					`Seconds left in grace period for previous auth: ${
						(old_auth_grace_period_in_ms - (Date.now() - public_auth?.created)) / 1000
					}`,
				);

				// log out all condition values
				log(
					`completely_invalid: ${completely_invalid}, outside_grace_period: ${outside_grace_period}, current_auth_is_valid: ${current_auth_is_valid}, previous_auth_is_valid: ${previous_auth_is_valid}, previous_auth_within_grace_period: ${previous_auth_within_grace_period}`,
				);
		...
		if (event?.game_config?.duration)
			new_challenge_expires_in_mins =
				new_challenge_expires_in_mins + event?.game_config?.duration / 60;
		...
		if (event.challenges.includes("naive")) {
			...
			const claim_code = await get_code_by_challenge(
				{ challenge_code: challenge_auth.token },
				{ app: "override captcha checks, string content irrelevant" },
			);
		...
		if (is_test_event || CI) {
			redirect_link += `?event_expires=${new_auth_expires_in_m * 60}s&chal_expires=${
				new_challenge_expires_in_mins * 60
			}s&grace=${old_auth_grace_period_in_ms / 1000}s&trail=`;
