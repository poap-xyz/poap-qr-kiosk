...
// Secrets
const { POAP_API_KEY, AUTH0_CLIENT_ID, AUTH0_ENDPOINT, AUTH0_CLIENT_SECRET, AUTH0_AUDIENCE } =
	process.env;
...
async function getAccessToken() {
	// Get API secrets
	const { access_token, expires } = await db
		.collection("secrets")
		.doc("poap-api")
		.get()
		.then(dataFromSnap);
...
exports.call_poap_endpoint = async (endpoint = "", data, method = "GET", format = "json") => {
	...
	log(
		`Calling ${apiUrl}${endpoint} with token ${access_token?.slice(0, 10)} and data ${
