...
exports.export_emails_of_static_drop = async (data, context) => {
	...
	try {
		...
		const { valid } = await call_poap_endpoint(
			`/event/validate`,
			{ event_id: drop_id, secret_code },
			"POST",
			"json",
		);
...
exports.delete_emails_of_static_drop = async (data, context) => {
	...
	try {
		...
		const { docs: emails_to_delete } = await db
			.collection(`static_drop_claims`)
			.where("drop_id", "==", `${drop_id}`)
			.get();
...
exports.update_public_static_drop_data = async function (change, context) {
	...
	return db
		.collection("static_drop_public")
		.doc(drop_id)
		.set(public_data_object, { merge: true });
};
