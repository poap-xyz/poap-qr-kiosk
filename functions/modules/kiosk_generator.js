...
app.post("/generate/:event_id", async (req, res) => {
	...
	const {
		generate_new_event_public_auth,
		validate_and_write_event_codes,
		get_event_template_by_code,
	} = require("./events");
	...
	try {
		...
		if (!event_id || !secret_code || !email)
			throw new Error(
				`Your email client does not support generating QR kiosks. Please create one manually at qr.poap.xyz.`,
			);
		...
		const codes = await call_poap_endpoint(
			`/event/${event_id}/qr-codes`,
			{ secret_code },
			"POST",
		);
		...
		if (codes.error) {
			...
			throw new Error(
				`Error in POAP codes API: ${codes.error}. This is probably not your fault.`,
			);
		}
		...
		if (existing_kiosk) {
			...
			old_codes = await db
				.collection("codes")
				.where("event", "==", existing_kiosk.uid)
				.get()
				.then(dataFromSnap);
