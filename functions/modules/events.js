 ...
 */
async function validate_and_write_event_codes(
	event_id,
	expiration_date,
	codes,
	existing_codes = [],
	...
	log(
		`Writing ${codes?.length} (of which ${existing_codes?.length} old) for event ${event_id} (expired ${expiration_date})`,
	);
	...
	const saneCodes = codes
		.map((code) => {
			...
			// Remove web prefixes
			qr_hash = qr_hash.replace(/(https?:\/\/.*\/)/gi, "");
	...
	const new_codes = saneCodes.filter(
		({ qr_hash }) => !existing_codes?.find((existing_code) => existing_code.qr_hash == qr_hash),
	);
...
async function get_event_template_by_code(an_event_qr_hash) {
	try {
		...
		const { event } = await call_poap_endpoint(`/actions/claim-qr`, {
			qr_hash: an_event_qr_hash,
		});
...
const update_event_data_of_kiosk = async (kiosk_id, public_kiosk_data) => {
	...
	try {
		...
		if (!public_kiosk_data)
			public_kiosk_data = await db
				.collection("publicEventData")
				.doc(kiosk_id)
				.get()
				.then(dataFromSnap);
		...
		if (dropId)
			event = is_mock ? mock_event() : await call_poap_endpoint(`/events/id/${dropId}`);
		...
		if (!dropId) {
			...
			const { event: event_of_code } = is_mock_code
				? { event: mock_event() }
				: await call_poap_endpoint(`/actions/claim-qr`, { qr_hash: code?.uid });
...
exports.registerEvent = async (request) => {
	...
	try {
		...
		const {
			...
			codes = [],
			challenges = [],
			game_config = { duration: 30, target_score: 5 },
			css,
		...
		if (!date.match(/^\d{4}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/))
			throw new Error("Please specify the date in YYYY-MM-DD, for example 2021-11-25");
		...
		const public_auth_expiry_interval_minutes = is_mock ? 0.5 : 2;
		const event_data = {
			dropId,
			name,
			email,
			...
			public_auth: generate_new_event_public_auth(
				public_auth_expiry_interval_minutes,
				is_mock,
			),
			created: Date.now(),
			updated: Date.now(),
		...
		if (new_event_id) {
			...
			await db
				...
				.doc(new_event_id)
				.delete()
				.catch((e) => log(`Error deleting event data for ${new_event_id}`, e));
			await db
				.collection("codes")
				.where("event", "==", new_event_id)
				.get()
				.then((snap) => snap.map((doc) => doc.ref.delete()))
				.catch((e) => log(`Error deleting event data for ${new_event_id}`, e));
		}
...
exports.updatePublicEventData = async function (change, context) {
	...
	if (before_data) {
		...
		const keys_changed = public_keys.some(
			(key) => JSON.stringify(after_data[key]) != JSON.stringify(before_data[key]),
		);
...
exports.deleteEvent = async function (data, context) {
	try {
		...
		const { authToken: eventAuthToken } = await db
			.collection("events")
			.doc(eventId)
			.get()
			.then(dataFromSnap);
...
exports.delete_data_of_deleted_event = async function (snap, context) {
	...
	try {
		...
		const { docs: codes_snap } = await db
			.collection("codes")
			.where("event", "==", eventId)
			.get();
		const { docs: challenges_snap } = await db
			.collection("claim_challenges")
			.where("eventId", "==", eventId)
			.get();
		const { docs: kiosk_scans } = await db
			.collection("scans")
		...
		const deletion_queue = [...codes_snap, ...challenges_snap, ...kiosk_scans].map(
			(doc) => () => doc.ref.delete(),
		);
		await throttle_and_retry(
			deletion_queue,
			maxInProgress,
			`delete data of deleted event`,
			5,
			5,
		);
	} catch (e) {
	...
	try {
		if (!process.env.development)
			return console.error(
				"getUniqueOrganiserEmails called externally which is never allowed",
			);
