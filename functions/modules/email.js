...
exports.sendEventAdminEmail = async ({ email, event }) => {
	...
	try {
		...
		const msg = {
			...
			text: (await fs.readFile(`${__dirname}/../templates/kiosk-created.email.txt`, "utf8"))
				.replace("%%eventlink%%", event.eventlink)
				.replace("%%adminlink%%", event.adminlink)
				.replace("%%eventname%%", event.name),
			html: await compilePugToEmail(
				`${__dirname}/../templates/kiosk-created.email.pug`,
				event,
			),
		};
...
exports.sendCustomClaimEmail = async ({ email, event, claim_code, html }) => {
	...
	try {
		...
		substituted_html = substituted_html.replace(/%%claim_code%%/gi, claim_code);
