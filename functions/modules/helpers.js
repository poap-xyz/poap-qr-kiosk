...
const allow_only_these_properties = (obj, allowed_properties) => {
	...
	if (unknownProperties.length)
		throw new Error(`Unknown properties given: ${unknownProperties.join(", ")}`);
};
...
// Wallet regexes
const wallet_regex = /^0x[a-z0-9]{40}$/gi;
const wallet_or_ens_regex = /^0x[a-z0-9]{40}|.*\.eth$/gi;
 ...
 */
function make_retryable(
	async_function,
	logging_label = "unlabeled retry",
	retry_times = 5,
	cooldown_in_s = 10,
	...
	const retryable_function = () =>
		Retrier((do_retry, retry_counter) => {
			// Failure handling
 ...
 */
async function throttle_and_retry(
	async_function_array = [],
	max_parallell = 2,
	logging_label = "",
	retry_times = 1,
	cooldown_in_s = 2,
	...
	const retryable_async_functions = async_function_array.map((async_function) => {
		const retryable_function = make_retryable(
			async_function,
			logging_label,
			retry_times,
			cooldown_in_s,
