...
exports.mint_code_to_address = async ({ data }) => {
	try {
		...
		while (!claim_success && claim_attempts < max_claim_attempts) {
			log(
				`Attempting to claim code ${code_to_claim} to address ${address_to_mint_to}, attempt: ${claim_attempts}`,
			);
			...
			const {
				claimed,
				secret,
				error: preclaim_error,
				message: preclaim_error_message,
			} = await call_poap_endpoint(`/actions/claim-qr`, { qr_hash: claim_code });

			// Handle claim secret errors
			if (preclaim_error) {
				log(`Error getting claim secret: `, preclaim_error);
				if (preclaim_error_message.includes("Qr Claim not found"))
					throw new Error(`Failed to get claim secret: Invalid claim link`);
				throw new Error(
					`Failed to get claim secret: ${preclaim_error_message || preclaim_error}`,
				);
			}
			...
			const { error, message } = await call_poap_endpoint(
				`/actions/claim-qr`,
				{
					secret,
