...
const checkCodeStatus = async (code) => {
	...
	const dayMonthYear = `${tomorrow.getDate()}-${
		tomorrow.toString().match(/(?:\w* )([A-Z]{1}[a-z]{2})/)[1]
...
async function claim_code_to_address(
	claim_code,
	drop_id,
	address,
	claim_secret,
	...
	log(
		`Claiming ${claim_code} to ${address} with ${
			send_default_email ? "default" : "custom"
		} email`,
	);
	...
	const claim_result = is_mock_claim
		? {}
		: await call_poap_endpoint(
				`/actions/claim-qr`,
				{
					address,
					qr_hash: claim_code,
					secret: claim_secret,
					sendEmail: send_default_email,
				},
				"POST",
				"json",
		  );
	...
	if (error)
		throw new Error(`${error}: ${message || Message || statusCode || "unknown details"}`);
	...
	const claim_meta = {
		...
		is_mock_claim,
		...(is_mock_claim && { expires: Date.now() + one_hour }),
	};
	await db
		.collection(`static_drop_claims`)
		.add({ address, drop_id, claim_code, claim_secret, ...claim_meta });
...
exports.refresh_unknown_and_unscanned_codes = async (event_id, context) => {
	...
	try {
		...
		const { started } = await db
			.collection("meta")
			.doc(`event_refresh_debounce_${event_id}`)
			.get()
			.then(dataFromSnap);

		// If there is a running refresh, and it is within the debounce window, exit
		if (started && started < Date.now() - debounce_ms)
			return log(`Refresh already running for ${event_id}`);

		// Set this run as the running one
		await db
			.collection("meta")
			.doc(`event_refresh_debounce_${event_id}`)
			.set({ started: Date.now(), started_human: new Date().toString() }, { merge: true });

		// Get old unknown codes
		const oldUnknowns = await db
			.collection("codes")
		...
		const [clean, withErrors] = oldUnknowns.reduce(
			(acc, val) => {
		...
		const olderWithErrors = withErrors.filter(
			({ updated }) => updated < Date.now() - ageInMs * errorSlowdownFactor,
		);
		...
		const old_unknown_queue = code_uids_to_update_deduped.map(
			(uid) =>
				function () {
		...
		await db
			...
			.set(
				{
					started: deleteField(),
					started_human: deleteField(),
					ended: Date.now(),
					ended_human: new Date().toString(),
				},
				{ merge: true },
			);
...
exports.refreshScannedCodesStatuses = async (eventId, context) => {
	...
	try {
		...
		const { started } = await db
			.collection("meta")
			.doc(`scanned_codes_debounce_${eventId}`)
			.get()
			.then(dataFromSnap);

		// If there is a running refresh, and it is within the debounce window, exit
		if (started && started < Date.now() - debounce_ms)
			return log(`Refresh already running for ${eventId}`);
		...
		const codeResetTimeout =
			expected_max_claim_duration + (event?.game_config?.duration || 1000 * 60);

		// Codes that have been scanned and have not been claimed
		const scannedAndUnclaimedCodes = await db
			.collection("codes")
			.where("event", "==", eventId)
			.where("scanned", "==", true)
			.where("claimed", "==", false)
		...
		const codesToReset = scannedAndUnclaimedCodes.filter(
			({ amountOfRemoteStatusChecks, lastRemoteStatusCheck }) =>
				amountOfRemoteStatusChecks > checkCodesAtLeast &&
				lastRemoteStatusCheck < Date.now() - codeResetTimeout,
		);
		const { throttle_and_retry } = require("./helpers");
		const reset_queue = codesToReset.map(
			({ uid }) =>
				() =>
		...
		const codesToCheck = scannedAndUnclaimedCodes.filter(
			({ updated }) => updated < Date.now() - checkCooldown,
		);

		// Build action queue
		const queue = codesToCheck.map(
			({ uid }) =>
				() =>
					updateCodeStatus(uid),
		);
...
const recalculate_available_codes = async (event_id) => {
	try {
		...
		// Throttle dependency
		const { throttle_and_retry } = require("./helpers");
		const code_statuses = await throttle_and_retry(
			update_queue,
			500,
			"recalculate_available_codes",
			2,
			5,
		);
		...
		await db
			...
			.set(
				{
					codesAvailable: codes_available,
					updated: Date.now(),
					updated_by: "recalculate_available_codes",
				},
				{ merge: true },
			);
...
exports.recalculate_available_codes_admin = async ({ data }) => {
	try {
		...
		const { authToken: valid_auth_token } = await db
			.collection("events")
			.doc(eventId)
			.get()
			.then(dataFromSnap);
...
exports.updateEventAvailableCodes = async function (change, context) {
	...
	if (edit_marker_after?.includes("recalculation") && edit_marker_before !== edit_marker_after) {
		if (debug)
			log(
				`DEBUG: edit marker changed from ${edit_marker_before} to ${edit_marker_after}, exiting`,
			);
		return;
	}
	...
	if (false === prevClaimed && ["unknown", true].includes(claimed)) {
		if (debug)
			log(
				`DEBUG: code was unclaimed, and it is now claimed or possibly claimed (unknown status), DECREMENT available`,
			);
		return db
			.collection("events")
			.doc(event)
			.set(
				{
					codesAvailable: increment(-1),
					updated: Date.now(),
					updated_by: "updateEventsAvailableCodes decrement",
				},
				{ merge: true },
			);
	}
	...
	if ([true, "unknown"].includes(prevClaimed) && claimed === false) {
		if (debug)
			log(
				`DEBUG: code was thought to be (possibly) claimed previously, but is unclaimed: INCREMENT available`,
			);
		return db
			.collection("events")
			.doc(event)
			.set(
				{
					codesAvailable: increment(1),
					updated: Date.now(),
					updated_by: "updateEventsAvailableCodes increment",
				},
				{ merge: true },
			);
	}
...
const get_code_for_event = async (event_id) => {
	...
	while (!valid_code) {
		...
		if (!oldestCode || !oldestCode.uid)
			throw new Error(`No more POAPs available for event ${event_id}!`);

		// Mark oldest code as unknown status so other users don't get it suggested
		log(
			`Marking code ${oldestCode.uid} claimed status as ${
				oldestCode.uid.includes("testing") ? true : "unknown"
			}: `,
			oldestCode,
		);
		await db
...
exports.get_code_by_challenge = async (data, context) => {
	try {
		...
		if (captcha_response) {
			...
			const { valid, expires, ...captcha_meta } = await db
		...
		if (!challenge || !challenge.eventId)
			throw new Error(
				`This link was already used by somebody else, scan the QR code again please`,
			);
		...
		if (challenge.expires < now_minus_grace_period) {
			log(
				`Challenge expired at ${challenge.expires} which is ${
					(now_minus_grace_period - challenge.expires) / 1000
				} seconds ago`,
			);
			throw new Error(
				`This link expired, please make sure to claim your POAP right after scanning the QR.`,
			);
		}
...
exports.claim_code_by_email = async (data, context) => {
	try {
		...
		if (!isEmail(email_or_0x_address) && !isWalletOrENS(email_or_0x_address))
			throw new Error(`Invalid email/wallet format`);
		if (!claim_code) throw new Error(`Missing event data`);

		// Remove all +hack elements of emails
		if (!isWallet(email_or_0x_address))
			email_or_0x_address = email_or_0x_address.replace(/(\+.*)(?=@)/gi, "");
		...
		if (is_static_drop) {
			const { custom_email: stored_custom_email } = await db
				.collection("static_drop_private")
				.doc(drop_id)
				.get()
				.then(dataFromSnap);
		...
		if (custom_email && isEmail(email_or_0x_address)) {
			...
			await sendCustomClaimEmail({
				email: email_or_0x_address,
				event,
				claim_code,
				html: custom_email,
			});
