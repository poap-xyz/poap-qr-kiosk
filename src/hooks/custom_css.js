...
export const useCustomCSS = () => {
	...
	const custom_css_paths = ["/event/", "/claim/", "/static/claim"];
	...
	// Event ID from code
	const { event: remote_event_by_code, drop_meta } = useCodeMetadata(
		claim_code,
		!should_grab_css,
	);
	...
	const event_id =
		local_store_event_id || stateEventId || routeEventId || remote_event_by_code?.id;
	...
	`;
	if (should_grab_css && !css_to_use.includes(`no_css_found`))
		log(`Custom CSS for ${event_id}: `, css_to_use);
