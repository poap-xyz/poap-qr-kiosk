			...
			(async () => {
                ...
                // Failure mode 1: Backend marked this device as invalid */
					...
					let { data: isValid } = await validateCallerDevice().catch((e) => {
						log(
							`Firebase gen2 functions can fail with 'Unauthorized' throws if appcheck fails`,
						);
						return { data: false };
					});
                ...
                // Failure mode 4: fallback captcha is not valid */
					...
					if (!isValid && captchaResponse) {
						...
						const { data: captchaIsValid } =
							await validateCallerCaptcha(captchaResponse);
