...
export default function ClaimPOAP() {
	...
	const { message: user_validation_status_message, user_valid } =
		useValidateUser(captchaResponse);
	...
	const { claim_link, error: error_getting_claim_code } = useClaimcodeForChallenge(
		captchaResponse,
		should_fetch_poap_claim_code,
	);

	log(`Challenge ${challenge_code}: `, challenge);
	log(
		`Has game: ${has_game_challenge}, Game done: ${gameDone}. User valid: ${user_valid} (${
			user_validation_status_message || "no message"
		}). Claim link: ${claim_link} (${
			should_fetch_poap_claim_code ? "should" : "should not"
		} fetch)`,
	);
  ...
  // /////////////////////////////*/
	...
	useEffect(() => {
		log(
			`Determining whether to forward. User is ${
				user_valid ? "valid" : "invalid"
			}, claim link is `,
			claim_link,
		);
	...
	if (user_validation_status_message || error_getting_claim_code)
		return <Loading message={user_validation_status_message || error_getting_claim_code} />;

	// If user is not valid, and no captcha response is known, show captcha
	if (!user_valid && !captchaResponse)
		return <Captcha onChange={(response) => setCaptchaResponse(response)} />;
	...
	if (user_valid && has_game_challenge)
		return (
			<Stroop
				duration_input={challenge?.game_config?.duration}
				target_score_input={challenge?.game_config?.target_score}
				onLose={handleLose}
				onWin={handleWin}
				poap_url={claim_link}
			/>
	...
	if (dev && user_valid && (gameDone || !has_game_challenge) && claim_link)
		return <Loading message={`POAP link: ${claim_link}`} />;
