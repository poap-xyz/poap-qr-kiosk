...
export default function StaticClaimAdmin() {
	...
	async function export_drop() {
		try {
			...
			const { data } = await export_emails_of_static_drop({
				drop_id,
				secret_code,
				auth_code,
			});
				...
				<Section>
					<Container>
						<CardContainer width="900px">
							...
							<Button
								download={`poap_drop_${drop_id}_email_claims-${new Date().toDateString()}.csv`}
								href={`data:text/csv;charset=utf-8,${csv_emails}`}
							>
								{t("staticAdmin.buttons.download_csv")}
							</Button>
						</CardContainer>
					</Container>
				</Section>
				...
				<Container>
					<CardContainer width="900px">
						...
						<Input
							...
							label={t("staticAdmin.labels.drop_id.label")}
							info={t("staticAdmin.labels.drop_id.info")}
						/>
						<Input
							type="text"
							value={secret_code}
							onChange={({ target }) => set_secret_code(target.value)}
							label={t("staticAdmin.labels.secret_code.label")}
							info={t("staticAdmin.labels.secret_code.info")}
						/>
						<Input
							type="text"
							value={auth_code}
							onChange={({ target }) => set_auth_code(target.value)}
							label={t("staticAdmin.labels.auth_code.label")}
							info={t("staticAdmin.labels.auth_code.info")}
						/>
						<Row>
							<Col>
								<Button onClick={export_drop}>
									{t("staticAdmin.buttons.export_drop")}
								</Button>
							</Col>
							<Col>
								<Button
									onClick={delete_drop_emails}
									margin="0 0 0 var(--spacing-2)"
								>
									{t("staticAdmin.buttons.delete_drop_emails")}
								</Button>
							</Col>
						</Row>
					</CardContainer>
