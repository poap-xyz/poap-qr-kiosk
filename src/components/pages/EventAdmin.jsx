...
export default function EventAdmin() {
	...
	const adminLink = `${
		dev ? "http://localhost:3000" : VITE_publicUrl
	}/#/event/admin/${eventId}/${authToken}`;
	...
	useEffect(() => {
		...
		if (eventId)
			return listenToEventMeta(eventId, async (event) => {
				setEvent(event);
	...
	async function safelyDeleteEvent() {
		try {
			...
			serveToast({
				message: `${t("eventAdmin.errorDeleteDispenser", { message: e.message })}`,
				type: "error",
			});
	...
	async function recalculateAvailableCodes() {
		try {
			...
			const {
				data: { error },
			} = await recalculate_available_codes({ eventId, authToken });
	...
	if (!event.loading && !event.codes)
		return (
			<Layout hide_background>
	...
	return (
		<Layout hide_background>
				...
				<Container width="760px">
					...
					<Grid>
						...
						<MethodCard
							...
							eventLink={eventLink}
							adminLink={adminLink}
							onDelete={handleOpenModalDestroy}
							onRecalculate={handleOpenModalRecalculate}
						/>
					</Grid>
				</Container>
			</Section>
			...
			<ModalSide open={isModalRecalculate} setIsOpen={setIsModalRecalculate} showClose={true}>
				<ModalContainer>
					<H3 color="var(--primary-600)" margin="0 0 var(--spacing-4) 0">
						Refresh counter
					</H3>
					<Divider margin="0 0 var(--spacing-7) 0" />
					<Text>
						Are you sure you want to refresh the mint link counter? Your POAP Kiosk will
						be unstable and might not work as expected.
					</Text>
				</ModalContainer>

				<Divider
					plain
					style={{ margin: "auto 0 1rem 0", borderTop: "1px solid var(--primary-200)" }}
				/>

				<ModalButtonContainer>
					<Button variation="white" onClick={handleCloseModalRecalculate}>
						Cancel
					</Button>
					<Button id="recalculateButton" onClick={recalculateAvailableCodes}>
						Refresh
					</Button>
				</ModalButtonContainer>
			</ModalSide>
			...
			<ModalSide open={isModalDestroy} setIsOpen={setIsModalDestroy} showClose={true}>
				<ModalContainer>
					...
					<Divider margin="0 0 var(--spacing-7) 0" />
					<Text>
						Are you sure you want to delete this kiosk? You can create a new kiosk for
						this POAP after deletion.{" "}
					</Text>
				</ModalContainer>

				<Divider
					plain
					style={{ margin: "auto 0 .5rem 0", borderTop: "1px solid var(--primary-200)" }}
				/>

				<ModalButtonContainer>
					<Button variation="white" onClick={handleCloseModalDestroy}>
						Cancel
					</Button>
					<Button id="safelyDeleteButton" onClick={safelyDeleteEvent}>
						Delete
					</Button>
				</ModalButtonContainer>
			</ModalSide>
