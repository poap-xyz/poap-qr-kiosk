...
import {
	...
	trackEvent,
	get_emulator_function_call_url,
	log_kiosk_open,
...
export default function ViewQR() {
	...
	useEffect(() => {
		...
		// if no event id in URL, exit
		if (event_id_is_cached)
			return log("URL indicates cached eventId, leaving localStorage as is");
	...
	useEffect(() => {
		try {
			if (eventId && !event_id_is_cached)
				return log(`Event ID present in url, ignoring localstorage`);

			log(
				`${
					event_id_is_cached ? `Event ID is cached` : `No event ID in url`
				} loading event ID from localstorage`,
			);
	...
	useEffect(() => {
		if (!internalEventId || internalEventId == "cached")
			return log(`No internal event ID, cancelling manual code refresh`);
	...
	useInterval(() => {
		if (!internalEventId || internalEventId == "cached")
			return log(`No internal event ID, cancelling scanned code refresh`);
	...
	useEffect(
		(f) => {
			...
			log(
				`For manual testing: \n${
					dev ? get_emulator_function_call_url("claimMiddleware") : VITE_publicUrl
				}/claim/${internalEventId}/${event?.public_auth?.token}?CI=true`,
			);
		},
	...
	if (iframeMode && event?.expires && event.expires < Date.now())
		return (
			<CustomCssWrapper>
	...
	// // No code qr error
	if (iframeMode && !event?.public_auth?.expires)
		return (
			<CustomCssWrapper>
	...
	// Expired event error
	if (event?.expires && event.expires < Date.now())
		return (
				...
				<Section>
					<Container>
						<CardContainer width="400px">
							...
							<br />
							<Text align="center">
								{t("eventView.expired.description", {
									expireDate: new Date(event.expires).toString(),
								})}
							</Text>
						</CardContainer>
					</Container>
				...
				<Section>
					<Container>
						<CardContainer width="400px">
							...
							<Button
								id="event-view-accept-disclaimer"
								onClick={(f) => setAcceptedTerms(true)}
							>
								{t("eventView.terms.acceptBtn")}
							</Button>
						</CardContainer>
					</Container>
				</Section>
	...
	return (
		<ViewWrapper
			...
			hide_footer
			center
			background={template?.footer_icon}
			show_bookmark
		>
			{/* Event metadata */}
			{event && (
				<H1 color={template?.main_color} align="center">
					{event.name}
				</H1>
			)}
			<H2 color={template?.header_link_color || "var(--primary-600)"} align="center">
				{t("eventView.display.subheading")}
			</H2>
