...
export default function Admin() {
	...
	const email_collect_options = [
		{
			label: t("eventCreate.form.dropCollectEmails.options", { returnObjects: true })?.[0]
				?.label,
			value: "no",
		},
		{
			label: t("eventCreate.form.dropCollectEmails.options", { returnObjects: true })?.[1]
				?.label,
			value: "yes",
		},
	];
		...
		(f) => {
			...
			(async (f) => {
				try {
					...
					// Validations
					const { name } = csv;
					if (!name.includes(".csv") && !name.includes(".txt"))
						throw new Error(`${t("eventCreate.file.acceptedFormat")}`);
					...
					data = data.map((code) => `${code}`.replace(/(https?:\/\/.*\/)/gi, "").trim());
					...
					if (erroredCodes.length) {
						...
						throw new Error(
							`${erroredCodes.length} ${t("eventCreate.file.codeFormat")} ${
								erroredCodes[0]
							}`,
						);
					}
					...
					const {
						data: { event, error },
					} = await getEventDataFromCode(formatedCode);
					...
					if (event.expiry_date) {
						...
						const endDate = `${year}-${monthNameToNumber(monthName)}-${
	...
	async function createEvent() {
		try {
			...
			if (!date.match(/^\d{4}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])$/))
				throw new Error(`${t("eventCreate.event.wrongDate")}`);

			// Confirm date
			const confirmed = confirm(
				`${t("eventCreate.event.creationMessage", { name: name, email: email })} ${new Date(
					date,
				).toLocaleDateString()} (${new Date(date)})`,
			);
	...
	return (
		<Layout
			id="event-create-layout-container"
			hide_background={codes}
			hide_footer
			onClick={() => setBackgroundTaps(backgroundTaps + 1)}
		>
					...
					>
						...
						<Row gap="1rem">
							<Col size={8}>
								<Input
									disabled
									label={t("eventCreate.form.fileName.label")}
									value={filename}
						...
						<Row>
							<Col size={1}>
								<Input
									disabled
									label={t("eventCreate.form.codesAmount.label")}
									value={codes.length}
						...
						<Input
							...
							placeholder={t("eventCreate.form.dropName.placeholder")}
							label={t("eventCreate.form.dropName.label")}
							toolTip={t("eventCreate.form.dropName.info")}
							value={name}
							optional
						/>
						...
						{dev || developer_mode ? (
							<Input
								...
								pattern="\d{4}-\d{2}-\d{2}"
								min={dateOnXDaysFromNow(1)}
								type="date"
								label={t("eventCreate.form.dropDate.label")}
								toolTip={`${t("eventCreate.form.dropDate.info")}`}
								value={date}
							/>
						) : null}
						...
						<Input
							...
							placeholder={t("eventCreate.form.dropEmail.placeholder")}
							label={t("eventCreate.form.dropEmail.label")}
							toolTip={t("eventCreate.form.dropEmail.info")}
							value={email}
						/>
						<Dropdown
						...
						/>
						{abuseProtection === "game" && (
							<Dropdown
								id="event-create-game-duration"
								handleOptionSelect={(option) => setGameDuration(option.value)}
								label={t("eventCreate.gameTime.label")}
								toolTip={t("eventCreate.gameTime.info")}
								options={t("eventCreate.gameTime.options", { returnObjects: true })}
							/>
						)}
						{developer_mode && (
							<Input
								...
								placeholder={t("eventCreate.form.dropCss.placeholder")}
								label={t("eventCreate.form.dropCss.label")}
								toolTip={t("eventCreate.form.dropCss.info")}
								value={css || ""}
							/>
						)}
						{developer_mode && (
							<Dropdown
								options={email_collect_options}
								id="event-create-collect-emails"
								handleOptionSelect={({ value }) =>
									setCollectEmails(value.includes("yes"))
								}
								label={t("eventCreate.form.dropCollectEmails.label")}
								toolTip={t("eventCreate.form.dropCollectEmails.info")}
								value={collectEmails ? "yes" : "no"}
							/>
						)}
						{developer_mode && !collectEmails && (
							<Input
								highlight={!customBaseurl}
								id="event-create-custom-baseurl"
								onChange={({ target }) => setCustomBaseurl(target.value)}
								placeholder={t("eventCreate.form.dropBaseurl.placeholder")}
								label={t("eventCreate.form.dropBaseurl.label")}
								toolTip={t("eventCreate.form.dropBaseurl.info")}
								value={customBaseurl || ""}
							/>
						)}
					</Grid>
					<FormFooter>
						<Button onClick={clearEvent} variation="white" tabIndex="0">
							Cancel
						</Button>
						{codes && (
							<Button id="event-create-submit" onClick={createEvent} tabIndex="0">
								{t("eventCreate.form.submitBtn", { count: codes.length })}
							</Button>
						)}
					</FormFooter>
				</Container>
