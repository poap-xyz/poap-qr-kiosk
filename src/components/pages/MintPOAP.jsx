...
export default function MintPOAP() {
	...
	useEffect(() => {
		...
		if (!address_to_mint_to?.length && probable_user_address)
			set_address_to_mint_to(probable_user_address);
	...
	async function handle_mint() {
		try {
			...
			if (
				!address_to_mint_to?.match(eth_or_ens_address_regex) &&
				!address_to_mint_to?.match(email_regex)
			) {
			...
			const {
				data: { error },
			} = await mint_code_to_address({ claim_code, address_to_mint_to, challenge_code });
			...
			<ViewWrapper center show_bookmark>
				<Container>
					<CardContainer width="400px" margin="0 auto">
						...
						>
							The minting process has started
						</H1>
						<Divider outline margin="0 0 var(--spacing-6) 0" />
						<Text align="center">
							Hundreds of computers around the world are working together to mint your
							POAP. When they are finished, this precious digital collectible will be
							yours forever on {address_to_mint_to}.
						</Text>
						<Text align="center">You can safely close this page.</Text>
						<Button href={`https://app.poap.xyz/scan/${address_to_mint_to}`}>
							View your POAP collection
						</Button>
					</CardContainer>
				</Container>
				{/* Confetti injection */}
		...
		<ViewWrapper center show_bookmark>
			<Container>
				<CardContainer width="400px" margin="0 auto">
					...
					<Divider outline margin="0 0 var(--spacing-6) 0" />
					<Input
						id="address-to-mint-to"
						label="Address to mint the POAP to:"
						placeholder="Enter your ETH address, ENS, or email"
						onChange={({ target }) => set_address_to_mint_to(target.value)}
						value={address_to_mint_to}
					/>
					<Button
						id="mint-poap-submit"
						onClick={handle_mint}
