...
const Timer = ({ duration, onComplete, ...props }) => {
	...
	return (
		<Text {...props}>
			{t("eventView.stroop.timer")} {duration - timePassed}
		</Text>
	);
};
...
export default ({ duration_input, target_score_input, onWin, onLose, poap_url, ...props }) => {
	...
	useEffect(() => {
		...
		if (target_score_input && target_score != target_score_input)
			set_target_score(target_score_input);
	...
	useEffect(
		() =>
			log(
				`Configuring game of ${duration} seconds, target_score: ${target_score}. onWin ${!!onWin}, onLose ${!!onLose}`,
			),
		[duration, target_score],
	);
	...
	if (done)
		return (
			<ViewWrapper center show_bookmark>
				<Container>
						...
						{score < target_score ? (
							<>
								...
								<H1
									align="center"
									size="var(--fs-lg)"
									margin="var(--spacing-5) 0 var(--spacing-1) 0"
								>
									{t("eventView.stroop.lost")}
								</H1>
								<Divider outline margin="0 0 var(--spacing-6) 0" />
								<Text align="center" margin="0 0 var(--spacing-5) 0" whiteSpace>
									{t("eventView.stroop.failMessage", {
										target_score: target_score,
									})}
								</Text>
								<Text align="center">
									{t("eventView.stroop.score")} <br /> {score}
								</Text>
								<Button onClick={() => setDone(false)}>
									{t("eventView.stroop.TryAgain")}
								</Button>
							</>
						) : (
							<>
								...
								>
									{" "}
									{t("eventView.stroop.won")}
								</H1>
								<Divider outline margin="0 0 var(--spacing-6) 0" />
								<Text align="center" margin="0 0 var(--spacing-6) 0">
									{t("eventView.stroop.score")} <br /> {score}
								</Text>
								<br />
								{poap_url ? (
									<Button
										onClick={claim_poap}
										leftIcon={<HeroIcon icon="sparkles" />}
									>
										{t("eventView.stroop.claimPoap")}
									</Button>
								) : (
									<AnimatedDots>{t("eventView.stroop.MintLoad")} </AnimatedDots>
								)}
							</>
						)}
					</CardContainer>
			...
			<ViewWrapper center show_bookmark>
				<Container>
					<CardContainer width="400px" margin="var(--spacing-8) auto 0 auto">
						...
						>
							{t("eventView.stroop.game.title")}
						</Text>
						<Divider outline />
						<Text color="var(--primary-700)" margin="1rem 0" align="center" whiteSpace>
							{t("eventView.stroop.game.subtitle")}
						</Text>

						<WebpImager imageUrl="/assets/decorations/Stroop-example" alt="test" />

						<Text align="center">
							{t("eventView.stroop.game.description", { duration: duration })}
						</Text>

						<Button width="100%" onClick={(f) => setStarted(true)}>
							{t("eventView.stroop.game.btn")}
						</Button>
					</CardContainer>
				</Container>
			</ViewWrapper>
	...
	return (
		<ViewWrapper center show_bookmark hide_background>
			<Container flex="1">
				<Grid width="400px" padding="var(--spacing-4) var(--spacing-1)">
					...
					>
						{t("eventView.stroop.playing.preTitle")} <br />{" "}
						<span style={{ color: getColorVariables(random_color(answer)) }}>
							{answer}
						</span>{" "}
						<br />
						{t("eventView.stroop.playing.postTitle")}
					</H1>
					<H2 align="center" color="var(--primary-600)" margin="0 0 var(--spacing-6) 0">
						{t("eventView.stroop.playing.subtitle", {
							score: score,
							target_score: target_score,
						})}
					</H2>
					{score >= target_score ? (
						<Text align="center" size="var(--fs-2xs)" color="#797292">
							{t("eventView.stroop.playing.keepGoing")}
						</Text>
					) : (
					...
					{options.map((option) => (
						<StroopButton
							...
							color="white"
							background={getColorVariables(option)}
							width="100%"
							margin="0 0 var(--spacing-7)"
							onClick={(f) => submit_answer(option)}
						>
							I am {option}
						</StroopButton>
					))}
