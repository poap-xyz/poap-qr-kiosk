...
import { POAPBookmark, mixin } from "@poap/poap-components";
...
export default ({
	...
	if (blank_until_custom_css)
		log(
			`Query paran lazy_css requested a blank page until custom CSS is checked: ${blank_until_custom_css}`,
			css,
		);
	...
	return (
		<>
			{show_bookmark && (
				<header style={{ height: "calc(var(--header-height) + 6px)" }}>
					<StyledBookmark />
				</header>
			)}

			<Main
				hideHeader={!show_bookmark}
				className={`${className} global_container`}
				{...props}
			>
				{hide_background ? (
					""
