...
export const get_claim_function_url = () => {
	...
	if (VITE_useEmulator !== "true") url = `${VITE_publicUrl}/claim`;
		...
		url = `http://localhost:${functions_emulator_port}/${VITE_projectId}/us-central1/claimMiddleware/claim`;
...
export async function extract_redirect_url(response) {
	cy.log(
		`Response object from which to extract challenge (can be printed to browser console): `,
		response,
	);
	...
	if (!redirects?.length) {
		cy.log(
			`No redirects found, full response body (status ${response.status}): `,
			response.body,
		);
		...
		response?.allRequestResponses?.forEach((unique_res) => {
			cy.log(
				`Status ${unique_res["Response Status"]} from ${unique_res["Request URL"]}`,
				unique_res["Response Body"],
			);
		});
		return null;
	}
