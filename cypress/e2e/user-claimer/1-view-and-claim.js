...
const {
	get_claim_function_url,
	check_if_code_is_expected,
	request_options,
} = require("../../support/e2e");
...
context("Claimer can view valid events", () => {
		...
		cy.request({
			...
			.then((event_1_first_challenge) => {
				...
				cy.get("#loading_text")
					.invoke("text")
					.then((text) => {
						expect(text).to.satisfy((mint_link) =>
							check_if_code_is_expected(mint_link, twoCodes),
						);
					});
			});
	});
	...
	it("Event 2: Creates event", function () {
		...
		cy.get("#admin-eventlink-public")
			.invoke("val")
			.as("event_2_publiclink")
			.then((f) => cy.log(this.event_2_publiclink));
		cy.get("#admin-eventlink-secret")
			.invoke("val")
			.as("event_2_secretlink")
			.then((f) => cy.log(this.event_2_secretlink));
	});
	...
	it("Event 2: Can view event", function () {
		...
		cy.get("svg[data-code]")
			.invoke("attr", "data-code")
			.as("event_2_public_auth_link")
			.then((f) => cy.log(`Event 2 public auth link: ${this.event_2_public_auth_link}`));
	});
	...
	it("Event 2: Successfully redirects to challenge link", function () {
		...
		cy.request({
			...request_options,
			url: `${get_claim_function_url()}/${this.event_2_public_auth_link}`,
		})
			.as(`request`)
			.then(extract_challenge_from_url)
			.then((event_2_first_challenge) => {
				...
				cy.get("#loading_text")
					...
					.then((text) => {
						expect(text).to.satisfy((mint_link) =>
							check_if_code_is_expected(mint_link, fiveCodes),
						);
					});
			});
	});
	...
	it("Event 2: failed appcheck foreward to manual captcha", function () {
		cy.get_challenge_from_qr_public_auth(
			this.event_2_public_auth_link,
			`challenge_for_failed_appcheck`,
		).then((challenge_string) => {
			// Visit the challenge link
