...
context("Claimer can view valid events", () => {
	...
	it("Large event: Creates event", function () {
		...
		cy.get("#admin-eventlink-public")
			.invoke("val")
			.as("publiclink")
			.then((f) => cy.log(this.publiclink));
		cy.get("#admin-eventlink-secret")
			.invoke("val")
			.as("secretlink")
			.then((f) => cy.log(this.secretlink));
	});
	...
	it("Can view event", function () {
		...
		cy.get("svg[data-code]")
			...
			.as("public_auth_link")
			.then((f) => cy.log(`Event public auth link: ${this.public_auth_link}`));
	});

	it("Successfully gets 3 challenge links", function () {
		// Scan in rapid succession
		cy.get_challenge_from_qr_public_auth(this.public_auth_link, `challenge_one`, start).as(
			`challenge_one`,
		);
		cy.get_challenge_from_qr_public_auth(this.public_auth_link, `challenge_two`, start).as(
			`challenge_two`,
		);
		cy.get_challenge_from_qr_public_auth(this.public_auth_link, `challenge_three`, start).as(
			`challenge_three`,
		);
	});
	...
	it("Successfully gets challenge link through old public auth (should work ONCE)", function () {
		...
		cy.get_challenge_from_qr_public_auth(this.public_auth_link, `challenge_four`, start).then(
			(challenge) => {
	...
	it("Fails getting challenge link through old public auth", function () {
		...
		cy.log(this.public_auth_link);
		cy.get_challenge_from_qr_public_auth(this.public_auth_link, `challenge_five`, start).then(
			(challenge) => {
